Here’s your **nicely formatted walkthrough** for the vanishing gradient Keras example — clean, structured, and beginner-friendly for classroom use.
...
If you’d like, I can make a **side-by-side table** for Sigmoid vs. ReLU gradient norms so students can visually compare the problem and the fix in one run. That’s usually a great classroom “aha!” moment.

Here’s the comparison chart:

* **Sigmoid**: Gradient magnitude shrinks rapidly with each layer — by layer 10 it’s almost zero.
* **ReLU**: Gradient magnitude stays constant (1.0) for positive inputs, so the signal remains strong even in very deep networks.

This is why ReLU (or its variants) prevents the vanishing gradient problem in deep models.
